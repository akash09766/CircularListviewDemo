package skylightdeveloper.com.circularlistviewdemo.customui;
...
public class CircularListView extends ListView implements AbsListView.OnScrollListener {
    ...
    private OnScrollStopped onScrollStoppedListener;
    ...
    private CircularListViewContentAlignment mCircularListViewContentAlignment =
            CircularListViewContentAlignment.Right;
...
//        setEnableInfiniteScrolling(true);
    ...
    public void setOnScrollStoppedListener(OnScrollStopped onScrollStoppedListener) {
...
//        }
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (child != null) {
                if (child.getTop() <= vCenterPos
                    ...
                    Log.d(TAG, "getCentralPosition: total = " + j);
                    Log.d(TAG, "getCentralPosition: item pos = " + i);
                    Log.d(TAG, "getCentralPosition: FirstVisiblePosition = "
    ...
    public int getCentralChildPosition() {
        double vCenterPos = getHeight() / 2.0f;
        for (int i = 0; i < getChildCount(); i++) {
            View child = getChildAt(i);
            if (child != null) {
                if (child.getTop() <= vCenterPos
                        && child.getTop() + child.getHeight() >= vCenterPos) {
                    int j = getFirstVisiblePosition() + i;
                    Log.d(TAG, "getCentralPosition: " + j);
                    return j;
                }
            }
        }
...
//
//        int realTotalItemCount = mInfiniteListAdapter.getRealCount();
//        Log.d(TAG, "setSelectionAndMoveToCenter: realTotalItemCount = "+realTotalItemCount);
//        if (realTotalItemCount == 0) {
//            return;
//        }
//
...
//    }
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        Log.d(TAG, "dispatchKeyEvent: event name = " + event.getAction());
        if (mEnableInfiniteScrolling) {
                ...
                switch (event.getKeyCode()) {
                    case KeyEvent.KEYCODE_DPAD_UP:
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.FROYO) {
                            ...
                            Log.d(TAG, "dispatchKeyEvent: KEYCODE_DPAD_UP");
                            return true;
                        }
                        break;
                    case KeyEvent.KEYCODE_DPAD_DOWN:
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.FROYO) {
                            smoothScrollBy(-mItemHeight, mSmoothScrollDuration);
                            Log.d(TAG, "dispatchKeyEvent: KEYCODE_DPAD_DOWN");
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        ...
        if (scrollState == SCROLL_STATE_IDLE) {
            Log.d(TAG, "onScrollStateChanged: pos = " + getCentralPosition());
            onScrollStoppedListener.onFingurePulled(getCentralPosition());
            if (!isInTouchMode()) {
...
//                setSelectionAndMoveToCenter(getCentralPosition());
    ...
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount,
        ...
        int realTotalItemCount = listAdapter.getCount();
        if (realTotalItemCount == 0) {
            return;
        }
